...
if __name__ == '__main__':
    setproctitle.setproctitle("fxy-inference-adaranker")
    ...
    result_file = output_path+'/results_inference.tsv'
    ...
    file_path = config['test_dataset_path']

    print('loading dataset at {}'.format(datetime.now().strftime("%d/%m/%Y %H:%M:%S")))
    test_dataset = MyDataset(config, path=file_path, dataset_type='test')
    print('finish loading dataset at {}'.format(datetime.now().strftime("%d/%m/%Y %H:%M:%S")))
    test_data = DataLoader(dataset = test_dataset, batch_size = config['batch_size'], shuffle = False, pin_memory=True, num_workers=config['num_workers'], persistent_workers=True)
    test_result = trainer.evaluate(test_data, load_best_model=saved, model_file=saved_path, show_progress=config['show_progress'])
